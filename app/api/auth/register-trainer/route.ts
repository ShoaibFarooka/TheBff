...
export const POST = async (req: NextRequest, res: NextResponse) => {
  ...
  if (existingTrainer) {
    return new Response(JSON.stringify({ error: `Trainer already exist with this email!` }), { status: 403 })
  }
  ...
  try {
    ...
    return new Response(JSON.stringify({ error: `Trainer registered successfully!` }), { status: 200 })
  } catch (error) {
    console.error("S3 Upload Error:", error);
    return new Response(JSON.stringify({ error: `Unable to register trainer!` }), { status: 500 })
  }
};
