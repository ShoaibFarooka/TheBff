...
const RegisterForm = ({ onSuccess, onFailure }: RegisterFormProps) => {
    ...
    const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
        ...
        const { username: name, email, phone, password, address } = Object.entries(Object.fromEntries(formData)).reduce((acc, [key, value]) => {
            if (key.includes('.')) {
        ...
        try {
            const res = await fetch("/api/auth", {
                ...
                body: JSON.stringify({ email, password, name, phone, address, signup: true }),
            });
