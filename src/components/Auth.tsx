...
const ResetPassword = dynamic(() => import('@/components/ResetPasswordModal'), { ssr: false, loading: () => <div> </div> })

interface IAuth { 
    signup?: boolean
}

const InputGroup = ({ label, type = 'text', name, placeholder, hidden = false, extras = {} } : { label: string, type?: string, name: string, placeholder?: string, extras?: Object, hidden?: boolean }) => {
    return !hidden ? (
        <div className="flex flex-col mt-3 w-full">
            <label className="text-white text-lg mb-1">{label}</label>
            <input {...extras} type={type} name={name} placeholder={placeholder ?? `Enter ${label}`} className="px-5 py-2 rounded-md bg-white/20 outline-white/80" required />
        </div>
    ) : null
}
...
const Auth = ({ signup = false } : IAuth) => {
    ...
    const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
        ...
        if (signup && (!name || !phone)) return toast.error('Please enter your name and phone number.')
        if (signup && phone.length < 10) return toast.error('Phone number should not be less than 10 characters.')
        ...
        try {
            ...
            if ( res.status == 200 ) {
                ...
                toast.success(data.message ?? (signup ? 'You have been signed up successfully. Redirecting...' : 'You have been logged in successfully.'))
                if(signup) router.push('/onboarding')
                else router.push(searchParams.get('cb') ?? '/')
            } else {
                const data = await res.json()
    ...
    <main className="mt-[5.5rem] min-h-[70vh] px-5 md:px-14 lg:px-40">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 w-full h-full backdrop-blur-md rounded-lg">
            ...
            <div className="col-span-1 w-full h-full relative hidden md:block">
                <Image src={authImage} width={500} className="max-h-full" alt="Auth" />

                <div className="absolute bottom-0 left-0 w-10/12 pl-10 pb-10">
                    <h2 className="text-3xl font-semibold text-[#FED25B]">
                        {
                            signup ? 'Sign Up and join for fitness' : 'Welcome back! Login to your account.'
                        }
                    </h2>
                    <p className="text-white mt-1 text-lg"> Sunt aute cupidatat excepteur aliquip ad enim tempor. </p>
                </div>
            ...
            <div className="col-span-1 h-full center flex-col text-white max-h-full overflow-auto">
                ...
                <div className="max-w-max my-3 mx-auto bg-white rounded-full flex p-1">
                    <Link href="/login">
                        <button className={`px-8 py-2 rounded-full ${signup ? 'bg-transparent text-black' : 'bg-blue-500 text-white'}`}>
                            Login
                        </button>
                    </Link>
                    <Link href="/signup">
                        <button className={`px-8 py-2 rounded-full ${signup ? ' bg-blue-500 text-white' : 'bg-transparent text-black'}`}>
                            Sign Up
                        </button>
                    </Link>
                </div>

                <p className="text-lg px-5 py-2">Lorem Ipsum is simply dummy text of the printing and typesetting industry.</p>

                <form className="mt-2 w-full px-4 md:px-20" onSubmit={handleSubmit}>
                    <InputGroup label="Name" name="username" hidden={!signup} />
                    <InputGroup label="WhatsApp Number" name="phone" hidden={!signup} extras={{ minLength: 10, maxLength: 10 }} />
                    <InputGroup label="Email" name="email" type="email" />
                    <InputGroup label="Password" name="password" type='password' extras={{ minLength: 8, maxlength: 12 }} />
                    {
