    ...
    <div className='h-full w-full px-10'>
        ...
        
      {
          posts?.length > 0 && posts.map((post, index: number) => (
              ...
              <div  className=" h-full flex flex-col items-start px-3 py-2 rounded-md bg-gray-100 text-black" key={"featured-post" + (index + 1)}>
                {/* href={`/blog/${post.slug}`} */}
                ...
                <div className=" my-auto cursor-pointer">
                  ...
                  <h3 className="text-xl text-gray-900 font-semibold mb-2 line-clamp-2">
                    {post?.title}
                  </h3>
                  <p className="text-gray-700 my-2 text-sm line-clamp-3">
                    {post?.excerpt}
                  </p>
                  <p className='text-gray-500 text-xs'> {new Date(post.date).toDateString()}</p>
                  </div>
                </Link>
